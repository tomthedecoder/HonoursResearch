...
def plot_all_neurons(individual, final_t=10, step_size=0.01):
    ...
    if type(individual) is not Individual:
        raise TypeError("Input must be of the individual class")
    ...
    individual.evaluate(final_t, reset=False, DT=step_size)
    ...
    if individual.num_nodes == 1:
        ...
        plt.xlabel("Time(t)")
        plt.ylabel("Output(y)")
        plt.plot(times, individual.ctrnn.history[0])
        plt.show()
        return
    ...
    for idx in range(int(np.ceil(individual.num_nodes / MAX_NODE))):
        nodes_in_plot = min(individual.num_nodes, MAX_NODE)
        fig, axs = plt.subplots(nodes_in_plot)
        for node in range(nodes_in_plot):
            axs[node].plot(times, individual.ctrnn.history[node + nodes_plotted])
